#!/usr/bin/env python
"""
Experiment 3: Multi-Seed Robustness — 60M model, n=100, 5 seeds
================================================================
...
Purpose: Establish statistical robustness of crystallization metrics.
Converts single-sample findings from Experiments 1-2 into claims with
error bars (mean +/- std across seeds).

Analyses per seed: Core metrics (R, H, rho) + seqsep + contact precision
"""
...
EXPERIMENT_NAME = "2026-03-01-multiseed-60m-n100"
ARTIFACTS_BASE = REPO_ROOT / "analysis_output" / "artifacts" / EXPERIMENT_NAME
...
def plot_aggregate(
    ...
    Args:
        ...
        R: [seeds, T, L, H] logit dominance.
        H: [seeds, T, L, H] entropy.
        rho: [seeds, T, L, H] spatial alignment, or None.
        prec_full, prec_b, prec_c: [seeds, T, L, H] contact precision, or None.
        seqsep_R: [seeds, bins, T, L, H] seqsep logit dominance, or None.
        protein_length: protein length (for entropy normalisation).
    """
    import matplotlib
    matplotlib.use('Agg')
    ...
    for l_idx, l in enumerate(layers):
        # Mean over heads first → [seeds, T], then mean/std over seeds → [T]
    ...
    fig.suptitle(f'Aggregate Crystallization (n={protein_length}, {n_seeds} seeds, shaded = ±1 std)',
                 fontsize=13, y=1.02)
    plt.tight_layout()
    ...
    if prec_full is not None:
        ...
        for ax, (data, title, color) in zip(axes, panels):
            for l_idx, l in enumerate(layers):
                d_per_seed = data[:, :, l, :].mean(axis=-1)  # [seeds, T]
        ...
        fig.suptitle(f'Contact Precision (n={protein_length}, {n_seeds} seeds)', fontsize=13)
        plt.tight_layout()
    ...
    if seqsep_R is not None:
        ...
        for bin_idx, (label, color) in enumerate(zip(bin_labels, bin_colors)):
            # Layer 0, mean over heads → [seeds, T]
        ...
        ax.set_title(f'Logit Dominance by Sequence Separation (Layer 0, {n_seeds} seeds)')
        ax.set_xlabel('Timestep (t)')
        ax.set_ylabel('R = ||B||/||C||')
        ax.legend()
        ax.grid(True, alpha=0.3)
...
def aggregate():
    ...
    print(f"\n{'='*60}")
    print(f"Aggregated results across {n_seeds} seeds")
    print(f"{'='*60}")
    ...
    # Core metrics: report layer 0 at t=0 and overall at t=1
    R_l0_t0 = R[:, 0, 0, :].mean(axis=-1)  # [seeds] mean over heads
    print(f"\nLogit Dominance (R) — Layer 0 at t=0:")
    print(f"  Mean: {R_l0_t0.mean():.3f} +/- {R_l0_t0.std():.3f}")

    H_all_t0 = H[:, 0, :, :].mean(axis=(-1, -2))  # [seeds]
    H_all_t1 = H[:, -1, :, :].mean(axis=(-1, -2))  # [seeds]
    ...
    if all_rho:
        ...
        rho_t1 = rho[:, -1, :, :].mean(axis=(-1, -2))  # [seeds]
    ...
    if all_seqsep_R:
        ...
        for bin_idx, label in enumerate(['local (1-6)', 'medium (7-23)', 'long (>=24)']):
            vals = seqsep_R[:, bin_idx, 0, 0, :].mean(axis=-1)  # [seeds]
            print(f"  {label:20s}: {vals.mean():.3f} +/- {vals.std():.3f}")

    if all_prec_full:
        prec_full = np.stack(all_prec_full)  # [seeds, T, L, H]
