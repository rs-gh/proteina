#!/usr/bin/env python
"""
Experiment 5: Causal Ablation of Pair Bias B
=============================================
...
Purpose: Test whether the geometric bias B is causally necessary throughout
generation, or only during early bootstrapping. We zero out B at specific
(layer, timestep) combinations and measure impact on structural quality.
...
6. deep_ablation   — B=0 at layers 6-11 only

Metrics: Radius of gyration, CA distance statistics, secondary structure
content (DSSP), contact map density, and optionally scRMSD.
"""
...
EXPERIMENT_NAME = "2026-03-01-bias-ablation"
...
SEEDS = [42, 123, 256]
PROTEIN_LENGTH = 100
NUM_SAMPLES = 1  # per seed (batch size 1 for analysis mode)
...
CONDITIONS = [
    ...
    AblationCondition(
        name="deep_ablation",
        description="B=0 at layers 6-11, all timesteps",
        ablate_layers={6, 7, 8, 9, 10, 11},
    ),
]
...
def run(ckpt_path: str = DEFAULT_CKPT):
    ...
    model.eval()
    model.to(device)
    print(f"Model loaded ({model.nn.nlayers} layers, "
          f"{model.nn.transformer_layers[0].mhba.mha.heads} heads)")
    ...
    for condition in CONDITIONS:
        ...
        for seed in SEEDS:
            ...
            # Enable capture minimally (just to have tracker plumbed through)
...
def save_summary(all_results: dict):
    ...
    with open(summary_path, "w") as f:
        ...
        f.write(f"Model: 60M, n={PROTEIN_LENGTH}, seeds={SEEDS}\n\n")
