#!/usr/bin/env python
"""
Experiment: Multi-Seed Robustness — 200M model (notri), n=100, 3 seeds
=======================================================================

Date: 2026-03-15
Model: proteina_v1.2_DFS_200M_notri (200M params, 15 layers, 12 heads)
Protein length: 100 residues
Seeds: [5, 42, 123]

Purpose: Replicate the 60M multiseed crystallization analysis on the 200M
model to enable cross-scale comparison. Runs core metrics (R, H, rho),
sequence separation analysis, and contact precision.
"""
...
EXPERIMENT_NAME = "2026-03-15-multiseed-200m-n100"
ARTIFACTS_BASE = REPO_ROOT / "analysis_output" / "artifacts" / EXPERIMENT_NAME
DEFAULT_CKPT = "checkpoints/proteina_v1.2_dfs_200m_notri_v1.0"
...
def plot_aggregate(
    ...
    fig.suptitle(f'200M Model — Aggregate Crystallization (n={protein_length}, {n_seeds} seeds, shaded = ±1 std)',
                 fontsize=13, y=1.02)
    plt.tight_layout()
    ...
    if prec_full is not None:
        ...
        fig.suptitle(f'200M Model — Contact Precision (n={protein_length}, {n_seeds} seeds)', fontsize=13)
        plt.tight_layout()
    ...
    if seqsep_R is not None:
        ...
        ax.set_title(f'200M — Logit Dominance by Sequence Separation (Layer 0, {n_seeds} seeds)')
        ax.set_xlabel('Timestep (t)')
        ax.set_ylabel('R = ||B||/||C||')
        ax.legend()
        ax.grid(True, alpha=0.3)
...
def aggregate():
    ...
    print(f"\n{'='*60}")
    print(f"200M Model — Aggregated results across {n_seeds} seeds")
    print(f"{'='*60}")
